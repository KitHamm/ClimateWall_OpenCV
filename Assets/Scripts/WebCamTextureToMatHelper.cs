...
namespace OpenCVForUnity.UnityUtils.Helper
{
    ...
    public class WebCamTextureToMatHelper : MonoBehaviour
    {
        ...
        public virtual string requestedDeviceName
        {
            ...
            set
            {
                if (_requestedDeviceName != value)
                {
                    ...
                    if (hasInitDone)
                        Initialize();
                    Debug.Log(_requestedDeviceName);
                }
            }
        }
        ...
        public virtual void Initialize(bool requestedIsFrontFacing, float requestedFPS = 60f, bool rotate90Degree = false)
        {
            if (isInitWaiting)
            {
                CancelInitCoroutine();
        ...
        /// <param name="requestedFPS">Requested FPS.</param>
        /// <param name="rotate90Degree">If set to <c>true</c> requested to rotate camera frame 90 degrees. (clockwise)</param>
        public virtual void Initialize(string deviceName, int requestedWidth, int requestedHeight, bool requestedIsFrontFacing = false, float requestedFPS = 60f, bool rotate90Degree = false)
        {
            if (isInitWaiting)
            {
                CancelInitCoroutine();
        ...
        protected virtual IEnumerator _Initialize()
        {
            ...
            while (true)
            {
                ...
                else if (webCamTexture.didUpdateThisFrame)
                {
                    ...
                    + " videoRotationAngle:" + webCamTexture.videoRotationAngle + " videoVerticallyMirrored:" + webCamTexture.videoVerticallyMirrored + " isFrontFacing:" + webCamDevice.isFrontFacing);
