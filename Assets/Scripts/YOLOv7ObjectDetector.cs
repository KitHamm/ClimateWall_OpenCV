...
namespace OpenCVForUnityExample.DnnModel
{
    ...
    public class YOLOv7ObjectDetector
    {
        ...
        public virtual Mat infer(Mat image)
        {
            ...
            int count = 0;
            //Find how many results are person
            for (int i = 0; i < num; ++i)
            {
            ...
            //Mat set to size of how many person detected
            Mat personResults = new Mat(count, 6, CvType.CV_32FC1);
            //Fill new Mat with person results and return
            for (int i = 0; i < num; ++i)
            {
                int[] classId_arr = new int[1];
                classIds.get(i, 0, classId_arr);
                ...
                if (id == 0)
                {
                    ...
                    personResults.put(i, 0, new float[] { x, y, x + w, y + h, confidence, id });
                }

            }

            //Debug.Log("Count: " + count + " | Rows: " + personResults.rows());
            //return results;
        ...
        public virtual void visualize(Mat image, Mat results, bool print_results, bool isRGB)
        {
            if (image.IsDisposed)
                return;
